...
class TransportPlayStopButtonView(View):
    ...
    def handle_ButtonPressedAction(self, action):
        if action.button == self.product_defs.FunctionToButton.get("TransportTogglePlayStop"):
    ...
    def update_led(self):
        ...
        self.button_led_writer.set_button_colour(
            self.product_defs.FunctionToButton.get("TransportTogglePlayStop"),
            Colours.button_toggle_on if playing else Colours.off,
        )

    def turn_off_led(self):
        self.button_led_writer.set_button_colour(
            self.product_defs.FunctionToButton.get("TransportTogglePlayStop"), Colours.off
        )
